...
- "Turdus merula" normalizes to `turdus-merula.png` plus any curated `-N` variants. The assets were renamed to modern eBird / BirdNET v2.4 names in a one-off migration; taxonomy has moved since, so `normalize` folds a reclassified species' two names to one key (the current one) and `variants_for` reaches back through the alias to the label the plate is actually filed under. Either spelling is a valid filename and both find the same artwork.
- `assets/birdnet_aliases.json` is that alias map: an unmodified copy of [OpenFauna](https://github.com/tphakala/openfauna)'s `build/aliases.json`, the same data BirdNET-Go canonicalizes with, so the frame agrees with the detector by construction rather than by a hand-kept guess. CC BY-SA 4.0, credited in `assets/ATTRIBUTION.md`. Refresh it by hand when a species is reported missing - a stale entry only means a plate is not found, which is the behaviour without it.
...
- `tests/test_artwork_names.py` enforces it: every filename (`perches/` aside) must be a BirdNET label or that label's current name, a hybrid (`-x-`), or a listed exception.
