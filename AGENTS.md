...
- "Turdus merula" normalizes to `turdus-merula.png` plus any curated `-N` variants. The assets were renamed to modern eBird / BirdNET v2.4 names in a one-off migration; taxonomy has moved since, so `normalize` folds a reclassified species' two names to one key (the current one) and `variants_for` gathers the files under both the current name and the label, current first. Either spelling is a valid filename, and a plate added under one never hides those filed under the other.
